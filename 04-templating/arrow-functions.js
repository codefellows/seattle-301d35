...
// Arrow functions are not only an alternate syntax; there is one key technical difference to keep in mind: arrow functions do not create their own scope, so they get their contextual "this" from the parent context.

// That means arrow functions CANNOT be used for constructor functions or to add methods to a constructor's prototype property.
...
let logger1 = input => console.log('logger1', input);
// In this case the right side of the arrow is an implicit return value that is assigned to the variable. This happens when there is only one statement in the function.

let logger2 = (input) => console.log('logger2', input);
// Some like to use optional parentheses around the parameter
